...
if(!isValidObjectId(userId)){
    return res.status(400).send({status:false,message:"please put a valid objectId"})
}
...
const removeProduct = cartDetails.item.productId
const updatedCart = await cartModel.findOneandUpdate({_Id:cartId},{$inc:{removeProduct: -1}})
return res.status(200).send({status:true,message:"cart updated sccessfully",data:updatedCart})
